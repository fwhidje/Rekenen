...
// Canonical subitising positions, including the empty case
...
interface DotPatternDecomposeMeta {
  showA:   boolean   // true → show 'a' as the known part, false → show 'b'
  options: number[]  // up to 4 candidate values for the unknown part
  stage:   Stage
}
...
function ChoiceButton({ value, colour, ink, paper, showDie, showNumber, onClick, disabled }: {
  ...
  return (
    <button
      ...
      {showDie && <Die n={value} colour={colour} ink={ink} paper={paper} size={56} />}
      {showNumber && (
        <span style={{ fontFamily: 'Fredoka One, cursive', fontSize: 26, color: ink, lineHeight: 1 }}>
          {value}
        </span>
      )}
    </button>
...
function DotPatternDecomposeComponent({ question, onResolve, disabled, scene }: ExerciseComponentProps<DotPatternDecomposeMeta>) {
  ...
  const handlePick = (v: number) => onResolve(v === unknownVal)

  const showDie    = stage !== 'number-number' && stage !== 'die-number'
  const showNumber = stage === 'die-mix' || stage === 'die-number' || stage === 'number-number'
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18, width: '100%' }}>
      ...
      {knownAsNum
        ? (
          <div style={{ fontFamily: 'Fredoka One, cursive', fontSize: 48, color: knownColour, lineHeight: 1 }}>
            {knownVal}
          </div>
        )
        : <Die n={knownVal} colour={knownColour} ink={ink} paper={paper} size={70} />
      }
      ...
      <div style={{
        ...
        {options.map(opt => (
          <ChoiceButton
            ...
            ink={ink} paper={paper}
            showDie={showDie} showNumber={showNumber}
            onClick={() => handlePick(opt)} disabled={disabled}
          />
        ))}
      </div>
    </div>
...
function makeOptions(correct: number, total: number): number[] {
  // pool = all possible part values in [0, total]
