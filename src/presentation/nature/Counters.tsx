...
import { bleedFilter } from './Defs'

export type Variant = 'heavy' | 'medium' | 'light'
...
const INK = '#3d2f1e'
const FLAT = {
  pollen: '#f0b932', pollenD: '#c79023',
  berry: '#c14b3a', berryD: '#8e2e22',
  leaf: '#7a9a3a', leafD: '#4a6b2a', leafL: '#a4c258',
  bloom: '#d97a85', bloomD: '#a64850',
  plum: '#8a5a99', plumD: '#5e3e6a',
  earth: '#8a6840', earthD: '#6a4f2e',
  paper: '#f4ecd8', cream: '#f3e6c4', wing: '#e3eef3',
}

function fillFor(name: keyof typeof FLAT, variant: Variant): string {
  if (variant === 'light') return FLAT[name]
  return `url(#wash-${name})`
}
function strokeW(variant: Variant, base = 2.4): number {
...
function Grain({ children }: { children: React.ReactNode }) {
  return (
...
export function Bee({ variant = 'medium', size = 110 }: CounterProps) {
  const rawId = useId().replace(/[^a-z0-9]/gi, '')
  const clipId = `bee-body-${rawId}`
  const fill = fillFor('pollen', variant)
  const sw = strokeW(variant)
  return (
    <svg viewBox="0 0 110 110" width={size} height={size}>
      <defs><clipPath id={clipId}><path d={BEE_BODY} /></clipPath></defs>
      <GroundShadow cx={54} cy={100} rx={30} />
      <g filter={bleedFilter(variant)}>
        <path d="M 30 78 Q 26 86 24 92" stroke={INK} strokeWidth={sw - 0.5} strokeLinecap="round" fill="none" />
        <path d="M 52 82 L 52 92" stroke={INK} strokeWidth={sw - 0.5} strokeLinecap="round" fill="none" />
        <path d="M 72 80 Q 76 86 78 92" stroke={INK} strokeWidth={sw - 0.5} strokeLinecap="round" fill="none" />
        ...
        <path d="M 60 46 C 78 36, 84 16, 72 12 C 58 14, 56 36, 58 46 Z"
          fill={FLAT.wing} stroke={INK} strokeWidth={sw - 0.6} opacity="0.65" strokeLinejoin="round" />
        <path d="M 50 46 C 28 40, 16 20, 32 10 C 52 6, 62 30, 56 46 Z"
          fill={FLAT.wing} stroke={INK} strokeWidth={sw - 0.4} opacity="0.85" strokeLinejoin="round" />
        <path d="M 50 44 Q 38 30 32 16" stroke={INK} strokeWidth="0.9" fill="none" opacity="0.45" strokeLinecap="round" />
        <path d="M 60 44 Q 70 30 72 16" stroke={INK} strokeWidth="0.9" fill="none" opacity="0.4" strokeLinecap="round" />
        ...
        <path d={BEE_BODY} fill="none" stroke={INK} strokeWidth={sw} strokeLinejoin="round" />
        <path d="M 32 52 Q 30 64 32 76" stroke={INK} strokeWidth={sw - 1} fill="none" opacity="0.3" strokeLinecap="round" />
        <ellipse cx={26} cy={60} rx={3.4} ry={3.8} fill={INK} />
        <circle cx={25} cy={59} r={1.1} fill={FLAT.paper} />
        <path d="M 20 68 Q 22 70 24 69" stroke={INK} strokeWidth="1" fill="none" strokeLinecap="round" opacity="0.7" />
        <path d="M 24 50 Q 18 36 14 26" stroke={INK} strokeWidth={sw - 0.5} fill="none" strokeLinecap="round" />
        <path d="M 30 48 Q 28 34 30 22" stroke={INK} strokeWidth={sw - 0.5} fill="none" strokeLinecap="round" />
        <circle cx={14} cy={26} r={2.2} fill={INK} />
        <circle cx={30} cy={22} r={2.2} fill={INK} />
      </g>
      {variant === 'heavy' && <Grain><path d={BEE_BODY} fill="#000" /></Grain>}
    </svg>
  )
}

export function Butterfly({ variant = 'medium', size = 110 }: CounterProps) {
    ...
    <svg viewBox="0 0 110 110" width={size} height={size}>
      ...
      <g filter={bleedFilter(variant)}>
        <path d="M 55 50 C 36 18, 8 22, 14 48 C 18 64, 38 64, 55 56 Z"
          fill={wingFill} stroke={INK} strokeWidth={sw} strokeLinejoin="round" />
        ...
        <path d="M 53 36 Q 50 36 50 40 L 50 88 Q 50 94 55 94 Q 60 94 60 88 L 60 40 Q 60 36 57 36 Z" fill={INK} />
...
export function Ladybug({ variant = 'medium', size = 110 }: CounterProps) {
  const sw = strokeW(variant)
  ...
  return (
    <svg viewBox="0 0 110 110" width={size} height={size}>
      ...
      <g filter={bleedFilter(variant)}>
        ...
        <path d={DOME} fill={fill} stroke={INK} strokeWidth={sw} strokeLinejoin="round" />
      ...
      {variant === 'heavy' && <Grain><path d={DOME} fill="#000" /></Grain>}
    </svg>
  )
}

export function Mushroom({ variant = 'medium', size = 110 }: CounterProps) {
  const sw = strokeW(variant)
  ...
  return (
    <svg viewBox="0 0 110 110" width={size} height={size}>
      ...
      <g filter={bleedFilter(variant)}>
        ...
        <path d="M 64 64 Q 70 78 66 99 L 69 99 Q 74 92 72 60 Z" fill={INK} opacity="0.12" />
...
export function Leaf({ variant = 'medium', size = 110 }: CounterProps) {
  const sw = strokeW(variant)
  ...
  return (
    <svg viewBox="0 0 110 110" width={size} height={size}>
      <GroundShadow rx={18} />
      <g filter={bleedFilter(variant)}>
        <path d="M 55 92 Q 56 80 62 70" fill="none" stroke={FLAT.earthD} strokeWidth={sw + 0.4} strokeLinecap="round" />
        ...
        <path d="M 62 70 C 54 58, 42 42, 22 26" fill="none" stroke={INK} strokeWidth={sw - 0.4} strokeLinecap="round" opacity="0.78" />
...
export function Daisy({ variant = 'medium', size = 110 }: CounterProps) {
  ...
  return (
    <svg viewBox="0 0 110 110" width={size} height={size}>
      ...
      <g filter={bleedFilter(variant)}>
        <path d="M 55 64 Q 55 80 52 100" fill="none" stroke={FLAT.leafD} strokeWidth={sw + 0.6} strokeLinecap="round" />
        ...
        {Array.from({ length: 8 }).map((_, i) => (
          <g key={i} transform={`rotate(${(i * 360) / 8} 55 46)`}>
            <ellipse cx={55} cy={22} rx={9} ry={14} fill={petal} stroke={INK} strokeWidth={sw - 0.4} strokeLinejoin="round" />
          </g>
        ))}
        <circle cx={55} cy={46} r={10} fill={center} stroke={INK} strokeWidth={sw - 0.2} />
      ...
      {variant === 'heavy' && <Grain><circle cx={55} cy={46} r={10} fill="#000" /></Grain>}
    </svg>
  )
}
