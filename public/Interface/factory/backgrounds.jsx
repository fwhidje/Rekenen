// Full-bleed scene backgrounds for the Factory theme.
// Outline rule: no outlines on background elements — paint-shapes only.

/* ─────────────────────── FACTORY INTERIOR ────────────────────────── */
// Industrial floor with conveyor belt, hanging lamps, pipes overhead.
function FactoryInteriorBackground({ children, style }) {
  return (
    <div style={{
      position: 'relative',
      width: '100%',
      ...
      <svg
        ...
        {/* hanging ceiling lamps — three of them */}
        {[80, 200, 320].map((cx, i) => (
          <g key={i}>
            {/* wire */}
        ...
        {/* conveyor belt across the floor */}
        <g>
        ...
        {/* floor markings — diamond hatch */}
        <g opacity="0.45">
          <path d="M 0 270 L 400 270" stroke="#2c241c" strokeWidth="0.8" />
          {Array.from({ length: 12 }).map((_, i) => (
            <path key={i}
            d={`M ${i * 36 - 10} 300 L ${i * 36 + 14} 264`}
            stroke="#2c241c" strokeWidth="0.7" />
          ))}
        </g>
...
/* ─────────────────────── FACTORY EXTERIOR ────────────────────────── */
// Sunset-tinted skyline with brick factory, smokestacks puffing.
function FactoryExteriorBackground({ children, style }) {
  return (
    <div style={{
      position: 'relative',
      width: '100%',
      ...
      <svg
        ...
        {/* distant haze of cooler buildings */}
        <g opacity="0.55">
        ...
        {/* main brick factory body */}
        <g>
          <rect x="100" y="148" width="200" height="80" fill="url(#factory-brick)" />
          {/* roofline shadow */}
          <rect x="100" y="148" width="200" height="6" fill="#3e1810" opacity="0.55" />
          {/* brick rows — faint horizontal lines */}
          {[160, 172, 184, 196, 208, 220].map((y, i) => (
            <line key={i} x1="100" y1={y} x2="300" y2={y}
            stroke="#3e1810" strokeWidth="0.6" opacity="0.45" />
          ))}
          {/* windows — 4 across, 2 rows, lit */}
          {[120, 160, 200, 240, 280].map((x) => (
            [166, 198].map((y, j) => (
              <g key={`${x}-${j}`}>
                ...
                <line x1={x + 7} y1={y} x2={x + 7} y2={y + 14}
                stroke="#1c1814" strokeWidth="0.8" />
                <line x1={x} y1={y + 7} x2={x + 14} y2={y + 7}
                stroke="#1c1814" strokeWidth="0.8" />
              </g>))
        ...
        {/* ground stripes (parking line) */}
        <g opacity="0.55">
          <rect x="20" y="262" width="40" height="4" fill="#e8c878" />
          <rect x="80" y="262" width="40" height="4" fill="#e8c878" />
          <rect x="140" y="262" width="40" height="4" fill="#e8c878" />
          <rect x="200" y="262" width="40" height="4" fill="#e8c878" />
          <rect x="260" y="262" width="40" height="4" fill="#e8c878" />
          <rect x="320" y="262" width="40" height="4" fill="#e8c878" />
        </g>
...
window.FactoryBackgrounds = { FactoryInteriorBackground, FactoryExteriorBackground };
