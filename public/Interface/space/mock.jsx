// Mocked exercise card — Space theme.
// Mirrors the structure of the Nature mock but uses space counters
// + StarfieldBackground.

function ExerciseMock({ Counter = null, counterSize = 70, counterOverlap = -14 }) {
  const { Rocket } = window.SpaceCounters;
  const { StarfieldBackground } = window.SpaceBackgrounds;
  const C = Counter || Rocket;
          ...
          <div style={{
            ...
            {['1','2','3','4','5','6','7','8','9','⌫','0','✓'].map(k => (
              <div key={k} style={{
                height: 44,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                background: k === '✓' ? '#5a6da8' : (k === '⌫' ? '#ecdfbe' : '#fbf6e6'),
                color: k === '✓' ? '#fff' : '#3d2f1e',
                border: '2px solid #3d2f1e',
                borderRadius: 14,
                fontFamily: 'Fredoka One, cursive',
